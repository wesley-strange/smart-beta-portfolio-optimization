...
def plot_covariance_returns_correlation(correlation, title):
    ...
    layout = go.Layout(
        ...
        width=800,
        height=800,
        xaxis=dict(domain=[.15, 1]),  # Primary x-axis
        yaxis=dict(domain=[0, .85]),  # Primary y-axis
        xaxis2=dict(domain=[0, .15], **xaxis2_layout),  # Secondary x-axis with domain and additional properties
        yaxis2=dict(domain=[.825, .975], **xaxis2_layout)  # Secondary y-axis with domain and additional properties
)
